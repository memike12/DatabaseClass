...
(function() {
  ...
  LightboxOptions = (function() {
    ...
    function LightboxOptions() {
      this.fileLoadingImage = '//cdn.symbolset.com/images/loading.gif';
        ...
        addSelectedChoice: function (data) {
            var enableChoice = !data.locked,
                enabledItem = $(
                    ...
                    "    <div></div>" +
                    "    <a href='#' class='select2-search-choice-close ss-icon' tabindex='-1'>delete</a>" +
                    "</li>"),
                disabledItem = $(
...
;(function () {
  ...
  function Royal (selector, sayings) {
    this.elem = selector.nodeType ? selector : document.querySelector(selector) // selector is a dom element
  ...
  Royal.prototype.type = function(saying) {
    ...
    // If saying is a string, make it an array now.
    // This can happen if this function is called directly.
  ...
  // Expose Royal to global namespace
  window.Royal = royal

})();


// Array filter polyfil
...
// jQuery JSON stringify method (https://gist.github.com/clementi/953765)
jQuery.extend({
...
$(function () {
  ...
  // Sticky footer
  // TODO: This should set a min-height so that the account page doesn't get messed up
  // when the modules are expanded.
  ...
  // Hide range slider if it's not supported
  ...
  var sayings = [
      ...
      "Made for the world web",
      "Turn text into images with magic",
      "Look sharp on Retina",
      "Computer and user friendly",
      "Download or cloud hosting",
      "It’s time to fast forward into the future ",
      "Edit and try for yourself",
      "Symbolset loves you"
    ]
  ...
  function switchColor () {
    if (length >= colors.length) length = 0;
    $('.view-brands .home-shop').css('background-color', colors[length++]);
    setTimeout(switchColor, 4600);
  }
  ...
  $('.set-licenses').on('change', function(){
    ...
    if (!in_cart) {
      cart.sets.push({'set_id': set_id, 'license_level': parseInt(licenses)})
    }
    ...
    if(price != 0){
      ...
      // Reset label to price of 1 license if none are selected
    ...
    // Add cents to label if it has any
  ...
  if ($('.sets-list .slideshow-wrapper').length) {
    $('.set-slideshow .slideshow-wrapper')
      .mouseover(function(){
        var that = this
  ...
  function startSlideshow (interval) {
    ...
    window.slideshowInterval = setInterval(function() {
      $(that).children('div').first()
        .fadeOut(750)
        .parents('.set-slideshow').css('background-color', colors[i]).end()
        .next()
        .fadeIn(750)
        .end()
        .appendTo(that)
  ...
  var editTitlePen = document.querySelector('.edit-title-here');
  var titlePlaceholder = document.querySelector('.title-field-placeholder');
  var newTitleInput = document.querySelector('.title-textarea');
  var projectRadio = document.querySelector('.sets-selection input[type=radio]');

  function enableTitleField(e){
    e.preventDefault();
    $(titlePlaceholder).addClass('is-hidden');
    $(editTitlePen).addClass('is-hidden');
    $(newTitleInput).removeClass('is-hidden');
    $(newTitleInput).focus().trigger('autosize.resize');
    $(newTitleInput).select();
  }

  $(titlePlaceholder).on('click', function(e){
    enableTitleField(e);
  });

  $(editTitlePen).on('click', function(e){
    enableTitleField(e);
  });

  $(newTitleInput).on('keyup', debounce(function(e){
    val = $(this).val();

    if ($.trim(val) != "" ){
      titlePlaceholder.innerHTML = val;
      $(this).attr('placeholder', val);
    }

    $(".js-save-project").removeClass("is-disabled");
  }, 200));

  $(newTitleInput).on('blur', function(e){
    val = $(this).val();
    ...
    $(titlePlaceholder).removeClass('is-hidden');
    $(editTitlePen).removeClass('is-hidden');
    $(newTitleInput).addClass('is-hidden');
  });
  ...
  $(newTitleInput).on('keydown', function(e){
    if (e.keyCode == 13){
      e.preventDefault();
      $(this).blur();
    }
  ...
  $('.sets-selection input[type=radio]').change(function(){
    $(".js-save-project").removeClass("is-disabled");
  });

  // TODO: Add keydown validation
  ...
  // Autoselect code snippet
  $('.account-embed').on('click', function(e){
    $(this).select();
  });

  $('.project-delete button').on('click', function (e) {
    e.preventDefault()
  ...
  $("input[name=set-selection]").change(function(){
    if ($(this).filter("[value=customized]").is(':checked')){
      $("#subsetter").removeClass("is-disabled");
    } else {
      $("#subsetter").addClass("is-disabled");
    }
    $(".js-save-project").removeClass("is-disabled");
  });
...
function cardValidation () {
  ...
  if (card && exp && cvc) {
    ...
    else cvc_el.removeClass('error');

    if (!cvc) cvc_el.addClass('error');
    else card_el.removeClass('error');

    if (!exp) { expm_el.addClass('error'); expy_el.addClass('error') }
    else { expm_el.removeClass('error'); expy_el.removeClass('error') }
...
function trackPurchase(){
  var total = Cookies.get('ss_purchase_total');
  var desc = Cookies.get('ss_purchase_description');

  if(total && desc && total != '' && desc != ''){
    ga('send', 'event', 'All', 'Purchased', desc, parseFloat(total));
    Cookies.expire('ss_purchase_total');
    Cookies.expire('ss_purchase_description');
  }
}
